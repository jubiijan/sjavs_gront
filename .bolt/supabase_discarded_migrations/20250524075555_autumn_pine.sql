/*
  # Game Functions Migration
  
  1. New Functions
    - validate_move: Validates if a card play is legal
    - evaluate_trick: Determines trick winner and points
    - calculate_scores: Updates player scores
    - manage_game_turn: Handles turn progression and game completion
    - play_card: Main function combining all game logic
  
  2. Changes
    - Fixed variable declaration in FOR loops
...
BEGIN
  ...
  FOR v_curr_player, v_curr_card IN 
    SELECT * FROM jsonb_each_text(v_game_state->'table_cards')
  LOOP
    -- Calculate card value
    DECLARE
      curr_suit CHAR := RIGHT(v_curr_card, 1);
      curr_rank TEXT := LEFT(v_curr_card, -1);
    ...
    BEGIN
      -- Permanent trumps
      IF (v_curr_card IN ('QC', 'QS', 'JC', 'JS', 'JH', 'JD')) THEN
        curr_value := 
          CASE v_curr_card
            WHEN 'QC' THEN 600
            WHEN 'QS' THEN 500
      ...
      IF (v_winning_card IN ('QC', 'QS', 'JC', 'JS', 'JH', 'JD')) THEN
        win_value := 
          CASE v_winning_card
            WHEN 'QC' THEN 600
            WHEN 'QS' THEN 500
      ...
      IF curr_value > win_value THEN
        v_winning_card := v_curr_card;
        v_winning_player := v_curr_player;
      END IF;
    END;
  END LOOP;
