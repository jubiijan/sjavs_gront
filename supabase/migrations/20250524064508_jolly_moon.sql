/*
  ...
  2. Security
    - All functions are database-level for consistency
...
BEGIN
  ...
  -- Iterate through each card and player
  FOR card_data IN 
    SELECT key AS player_name, value AS card
    FROM jsonb_each_text(v_game_state->'table_cards')
  LOOP
    -- Assign values from the record
    DECLARE
      curr_suit CHAR := RIGHT(card_data.card, 1);
      curr_rank TEXT := LEFT(card_data.card, -1);
    ...
    BEGIN
      -- Permanent trumps
      IF (card_data.card IN ('QC', 'QS', 'JC', 'JS', 'JH', 'JD')) THEN
        curr_value := 
          CASE card_data.card
            WHEN 'QC' THEN 600
            WHEN 'QS' THEN 500
...
BEGIN
  ...
  IF v_trick_number >= 8 THEN
    ...
    SELECT
      ...
      CASE 
        WHEN (SELECT COUNT(*) FROM lobby_players WHERE lobby_id = game_state.lobby_id) = 3 THEN '3-player'
        ELSE '4-player'
      END,
      NOW()
    FROM game_state
