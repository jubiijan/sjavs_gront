/*
  # Game State Version Control

  1. Changes
    - Create index on version column
    - Add advisory lock function
    - Add atomic action processing function

  2. Details
    - Adds functions for handling concurrent game state updates
    - Implements optimistic locking with version checks
    - Provides atomic action processing
*/

-- Create index on version
CREATE INDEX IF NOT EXISTS idx_game_state_version ON game_state(version);
